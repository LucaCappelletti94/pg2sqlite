//! Diesel-backed integration tests for expression translations that were
//! previously missing in the forward translator.
...
use schema::{numeric_values, overlay_samples, pair_values, time_samples};
...
fn test_at_time_zone_fixed_offset_semantic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let query = select_sql(&translated);
    assert!(
        !query.to_uppercase().contains("AT TIME ZONE"),
        "AT TIME ZONE should be rewritten, got: {query}"
    );
    assert!(
        query.to_lowercase().contains("datetime"),
        "Expected datetime(...) rewrite, got: {query}"
    );
    ...
    let rows: Vec<ShiftedTimeRow> = diesel::sql_query(query).load(&mut conn)?;
    assert_eq!(rows.len(), 2);
    assert_eq!(rows[0].id, 1);
