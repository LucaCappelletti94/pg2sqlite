...
fn bool_and_causes_error() {
    // bool_and has NULL semantics incompatible with MIN — must error, not
    // silently map.
...
fn bool_or_causes_error() {
    // bool_or has NULL semantics incompatible with MAX — must error, not
    // silently map.
...
fn every_causes_error() {
    // every is an alias for bool_and in PostgreSQL — must error, not silently
    // map to MIN.
