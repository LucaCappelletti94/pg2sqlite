...
The app doubles as the project's landing page. It is the canonical
demo we link from the README and the crate metadata, and the live URL
exercises the same pg2sqlite build that ships on crates.io (consumed
here via `path = "../.."` with `default-features = false` so the no_std
+ alloc build path is what users see).
...
checks. This crate is intentionally NOT a workspace member. It points
at pg2sqlite via a `path = "../.."` dep with `default-features = false`
so it inherits pg2sqlite's no_std + alloc build (and the
`[patch.crates-io]` sqlparser fork that makes the `visitor` feature
no_std-compatible).
