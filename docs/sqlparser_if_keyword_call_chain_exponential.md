...
`current_time` (along with `current_date`, `current_user`, `session_user`, `localtime`, ...) is a SQL reserved word that the parser treats specially in expression position - it can stand alone, take an optional precision in parens, or appear at the head of `(...)` group expressions during disambiguation.
