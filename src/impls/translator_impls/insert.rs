...
use crate::{
    impls::{
        ...
        shared_helpers::{
            ColumnReferences, ColumnRewrites, carries_default_keyword, declares_always_identity,
            extract_columns_from_expr, is_default_keyword, substituted_assignment_default,
...
/// Refuses a value written for a `GENERATED ALWAYS AS IDENTITY` column.
///
/// PostgreSQL answers `cannot insert a non-DEFAULT value into column` unless
/// the statement says `OVERRIDING SYSTEM VALUE`. The emitted schema cannot
/// enforce that for itself: in a `BEFORE INSERT` trigger SQLite has not
/// assigned the rowid yet and `NEW.id` reads the same whether the caller
/// supplied a value or left the key alone, so the statement is refused where
/// it is translated. `DEFAULT` is how PostgreSQL asks for the generated
/// value and is accepted.
fn reject_explicit_identity_values(
    insert: &Insert,
    schema: &ParserDB,
) -> Result<(), crate::errors::Error> {
    let TableObject::TableName(table_name) = &insert.table else { return Ok(()) };
    let Some(table) = resolve_translation_table(schema, table_name)? else { return Ok(()) };
    let always_generated: Vec<String> = table
        .columns(schema)?
        .filter(|column| declares_always_identity(&column.attribute().options))
        .map(|column| column.column_name().to_owned())
        .collect();
    if always_generated.is_empty() {
        return Ok(());
    }

    let column_names = insert_column_names(insert, table, schema)?;
    let Some(source) = insert.source.as_deref() else { return Ok(()) };
    let SetExpr::Values(values) = source.body.as_ref() else {
        return reject_generated_always_source(&always_generated, &column_names);
    };
    for row in &values.rows {
        for (index, value) in row.content.iter().enumerate() {
            let Some(name) = column_names.get(index) else { continue };
            if !always_generated.iter().any(|always| always.eq_ignore_ascii_case(name))
                || is_default_keyword(value)
            {
                continue;
            }
            return Err(crate::errors::Error::forward_refusal(format!(
                "cannot insert the value {value} into column {name}, which is GENERATED ALWAYS \
                 AS IDENTITY: PostgreSQL takes only DEFAULT there, and SQLite cannot tell a \
                 supplied key from an omitted one at write time, so the refusal is made here. \
                 Write DEFAULT, leave the column out of the insert, or declare the column \
...
/// Refuses an `INSERT ... SELECT` that writes a `GENERATED ALWAYS` column,
/// where the value is not a literal this can inspect.
fn reject_generated_always_source(
    always_generated: &[String],
    column_names: &[String],
) -> Result<(), crate::errors::Error> {
    let Some(named) = column_names
        .iter()
        .find(|name| always_generated.iter().any(|always| always.eq_ignore_ascii_case(name)))
    else {
        return Ok(());
    };
    Err(crate::errors::Error::forward_refusal(format!(
        "column {named} is GENERATED ALWAYS AS IDENTITY and this insert supplies it from a \
         query, which PostgreSQL refuses without OVERRIDING SYSTEM VALUE. Leave the column out \
         of the insert, or declare it GENERATED BY DEFAULT AS IDENTITY."
    )))
}
